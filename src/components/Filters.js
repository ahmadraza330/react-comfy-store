        ...
        <div>
          {
            categories.map((c, idx) => (
              <button onClick={updateFilters} name="category" type='button' className={`${category === c.toLowerCase() ? 'active' : null}`}  key={idx}>{c}</button>
            ))
          }
        </div>
      </div>
        ...
        <div className="colors">
          {
            colors.map((c, idx) => {
              if(c === 'all') {
                return <button key={idx} name='color' onClick={updateFilters} data-color="all" className={`${color === 'all' ? 'all-btn active' : 'all-btn'}`}>
                  all
                </button>
              }
               return <button key={idx} name="color" onClick={updateFilters} style={{background: c}} className={`${color === c ? 'color-btn active' : 'color-btn'}`} data-color={c}>
